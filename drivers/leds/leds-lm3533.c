...
static inline u8 lm3533_led_get_pattern_reg(struct lm3533_led *led, u8 base)
{
				 ...
				 struct attribute *attr, int n)
{
	...
	if (attr == &dev_attr_als_channel.attr || attr == &dev_attr_als_en.attr) {
				...
				struct lm3533_led_platform_data *pdata)
{
	...
	if (pdata->delay_on && pdata->delay_off) {
		...
		reg =
		    lm3533_led_get_pattern_reg(led,
					LM3533_REG_PATTERN_FALLTIME_BASE);
