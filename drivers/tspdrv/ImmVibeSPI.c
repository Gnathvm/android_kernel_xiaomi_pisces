...
static const unsigned char LRA_init_sequence[] = {
	...
	FEEDBACK_CONTROL_REG,
	FEEDBACK_CONTROL_MODE_LRA | FB_BRAKE_FACTOR_4X |
	LOOP_RESPONSE_MEDIUM | FEEDBACK_CONTROL_BEMF_LRA_GAIN3,
	Control1_REG, STARTUP_BOOST_ENABLED | DEFAULT_DRIVE_TIME,	/* From Xiaomi */
	Control2_REG,
	BIDIRECT_INPUT | SOFT_BRAKE | AUTO_RES_GAIN_HIGH |
	BLANKING_TIME_MEDIUM | IDISS_TIME_MEDIUM,
			 ...
			 const struct i2c_device_id *id)
{
	...
	do {
		drv2604_write_reg_val(LRA_autocal_sequence,
	...
	if ((status & DIAG_RESULT_MASK) == AUTO_CAL_FAILED) {
		...
		/* restore 0x1a */
		drv2604_write_reg_val(LRA_autocal_done_seq,
...
#if SKIP_LRA_AUTOCAL == 1
		if (drv2604_read_reg(RATED_VOLTAGE_REG) !=
		    LRA_init_sequence[43]) {
			printk(KERN_INFO
				"ImmVibeSPI_ForceOut_AmpEnable: Register values resent.\n");
			drv2604_write_reg_val(LRA_init_sequence,
...
/* From Xiaomi end */
	...
	if (adapter) {
		...
		if (client) {
			...
			DbgOut((DBL_VERBOSE,
				"drv2604: Cannot create new device.\n"));
			return VIBE_E_FAIL;
		}
	} else {
		DbgOut((DBL_VERBOSE, "ImmVibeSPI_ForceOut_AmpDisable.\n"));
...
/* From Xiaomi start */
	if (timed_output_dev_register(&to_dev) < 0) {
		printk(KERN_ALERT
			"drv2604: fail to create timed output dev: enable\n");
		gpio_direction_output(GPIO_VIBTONE_EN1, GPIO_LEVEL_LOW);
	...
	/* PWM */
	if (platform_add_devices
	    (pisces_pwm_devices, ARRAY_SIZE(pisces_pwm_devices)))
						...
						char *szDevName, int nSize)
{
	...
	sprintf(szRevision, " Rev:%d",
		(drv2604_read_reg(SILICON_REVISION_REG) &
